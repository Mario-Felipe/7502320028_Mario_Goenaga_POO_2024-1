...
public class TareaCrud implements ModeloCrud<Tarea> {
    ...
    private HashMap<String, Tarea> listTarea = new HashMap<>();    

    public TareaCrud() {
        this.listTarea = new HashMap<>();
    }

    @Override
    public void agregar(Tarea objeto) throws Exception {
        try {
            if (listTarea.containsKey(objeto.getIdentificadorTarea())) {
                throw new Exception("La tarea ya se encuentra registrado");
            }
            listTarea.put(objeto.getIdentificadorTarea(), objeto);
        } catch (Exception e) {
    ...
    public Tarea buscar(String codigo) throws Exception {
        try {
            if (!listTarea.containsKey(codigo)) {
                throw new Exception("La tarea no se encuentra registrada.");
            }
